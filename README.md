...
### 1.3 核心价值
智能排序当前页面的变量和方法，支持全页面排序和选择区域排序两种模式，按照统一的排序规则提升代码整洁度和可读性。
...
#### 🔄 全页面排序模式
- **触发条件**：用户没有选择任何文本，或选择区域不包含完整的方法/变量定义
- **排序范围**：对整个当前打开的Java文件进行排序
- **适用场景**：整体代码重构和规范化

#### 🎯 选择区域排序模式
- **触发条件**：用户选择了包含一个或多个完整方法/变量定义的文本区域
- **排序范围**：仅对选择区域内的元素进行排序
- **适用场景**：局部代码优化和整理
...
Refactor
  ↳ Rearrange Code
  ↳ 🔄 Sort All Members A-Z                    ← 全页面排序
  ↳ 🔄 Sort Selected Members A-Z              ← 选择区域排序（有选择时显示）
```

### 3.2 操作反馈
- **全页面成功**：`"Sorted 8 elements: 3 fields, 5 methods"`
- **选择区域成功**：`"Sorted 3 selected elements"`
- **无元素警告**：`"No sortable elements found"`
- **无效选择提示**：`"Selection does not contain complete elements"`
